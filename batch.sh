...
main() {
	...
	for i in batches/*; do
		...
		bsub -N -B -u syang39@mgh.harvard.edu -J "test_$i" -L "/bin/bash" -e logs/%J-errors.err -o logs/%J-outputs.out -q normal -n 1 -R 'rusage[mem=1000]' < (trap 'kill 0' SIGINT; . $main_loc/analysis.sh) 
		#my_jobs+=($!)
		#timed_print ${my_jobs[@]}
	done
