...
get_pairs_all() { #place all files into tmp, group them
	...
	for i in ${SOURCE_LOC}/*; do
        ...
        timed_print "name is $name***"
		local only_name=${name%.*} #removes last extention, ie bam or gz
        timed_print "only_name is $only_name***"
        timed_print "start the case statement for ${name#*.}***"
		case "${name#*.}" in #get extention
			bam) bam_to_fastq $only_name ;; 
			fq) 
                mv_fq "${SOURCE_LOC}/${name}" "tmp/${SOURCE}/${name}.gz" 
                timed_print "moved ${SOURCE_LOC}/${name} to tmp/${SOURCE}/${name}.gz" ;; 
                #move and zip for smaller size 
			fq.gz) 
                mv_fq "${SOURCE_LOC}/${name}" "tmp/${SOURCE}/${name}"
                timed_print "moved ${SOURCE_LOC}/${name} to tmp/${SOURCE}/${name}" ;;
                   #move and does not change data
		esac
		timed_print "moved $i"
	done 
...
main(){
	...
	for i in ${directories[@]}; do
		if [[ ! -d "$i" ]]; then 
			mkdir $i
            echo " $i folder has been created"
		fi
	done
